...
class TestEvaluationFunction(unittest.TestCase):
    ...
    def assertEqual_input_variations(self, response, answer, params, value):
        ...
        for variation in variation_definitions:
            ...
            if (response_variation != response) or (
                answer_variation != answer
            ):
                result = evaluation_function(
                    response_variation, answer, params
                )
                self.assertEqual(result.get("is_correct"), value)
                result = evaluation_function(
                    response, answer_variation, params
                )
                self.assertEqual(result.get("is_correct"), value)
                result = evaluation_function(
                    response_variation, answer_variation, params
                )
                self.assertEqual(result.get("is_correct"), value)
    ...
    def test_simple_polynomial_with_input_symbols_correct(self):
        ...
        params = {"strict_syntax": False, "symbols": [["longName", []]]}
    ...
    def test_simple_fractional_powers_correct(self):
        params = {
            "strict_syntax": False,
            "symbol_assumptions": "('g','positive') ('v','positive')",
        }
        fractional_powers_res = [
            "sqrt(v)/sqrt(g)",
            "v**(1/2)/g**(1/2)",
            "v**(0.5)/g**(0.5)",
        ]
        ...
        for response in fractional_powers_ans:
            for answer in fractional_powers_ans:
                self.assertEqual_input_variations(
                    response, answer, params, True
                )
    ...
    def test_invalid_user_expression(self):
        ...
        self.assertEqual(
            parse_error_warning(response) in result["feedback"], True
        )
    ...
    def test_equality_sign_in_answer_not_response(self):
        ...
        self.assertEqual(
            result["feedback"],
            "The response was an expression but was expected to be an equality.",
        )
    ...
    def test_equality_sign_in_response_not_answer(self):
        ...
        self.assertEqual(
            result["feedback"],
            "The response was an equality but was expected to be an expression.",
        )
    ...
    def test_numerical_comparison(self):
        ...
        with self.subTest(
            tag="Correct response, tolerance specified with atol"
        ):
            response = "6.73"
            answer = "sqrt(3)+5"
        ...
        with self.subTest(
            tag="Incorrect response, tolerance specified with atol"
        ):
            response = "6.7"
            answer = "sqrt(3)+5"
            params.update({"atol": 0.005})
        ...
        with self.subTest(
            tag="Correct response, tolerance specified with rtol"
        ):
            response = "6.73"
            answer = "sqrt(3)+5"
        ...
        with self.subTest(
            tag="Incorrect response, tolerance specified with rtol"
        ):
            response = "6.7"
            answer = "sqrt(3)+5"
            params.update({"rtol": 0.0005})
        ...
        with self.subTest(
            tag="Answer is not constant, tolerance specified with atol"
        ):
            response = "6.73"
            answer = "sqrt(3)+x"
            params.update({"atol": 0.005})
        ...
        with self.subTest(
            tag="Response is not constant, tolerance specified with rtol"
        ):
            response = "6.7+x"
            answer = "sqrt(3)+5"
            params.update({"rtol": 0.0005})
        ...
        with self.subTest(
            tag="Answer is not constant, tolerance specified with rtol"
        ):
            response = "6.73"
            answer = "sqrt(3)+x"
            params.update({"rtol": 0.0005})
    ...
    def test_latex_expression(self):
        params = {"strict_syntax": False, "is_latex": True}
        with self.subTest(tag="frac"):
            answer = "(x+1)/(x+2)"
            response = r"\frac{x+1}{x+2}"
            result = evaluation_function(response, answer, params)
            self.assertEqual(result["is_correct"], True)
        ...
        with self.subTest(tag="log"):
            answer = "log(x, 10)"
            response = r"\log_{10} x"
            result = evaluation_function(response, answer, params)
            self.assertEqual(result["is_correct"], True)
        with self.subTest(tag="logexp"):
            answer = "10G"
            response = r"10*e^{\ln{G}}"
            result = evaluation_function(response, answer, params)
            self.assertEqual(result["is_correct"], True)
        with self.subTest(tag="not latex"):
            answer = "10G"
            response = "10*exp(log(G))"
